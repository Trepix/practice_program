...
class RoverDisplacementTests {
    ...
    @Test
    void lookingToWestMoveForward() {
        Rover rover = createRoverAtOriginLookingTo(WEST);

        rover.process("f");

        assertThat(rover, is(new Rover(WEST, new Position(-1, 0))));
    }

    @Test
    void lookingToWestMoveBackward() {
        Rover rover = createRoverAtOriginLookingTo(WEST);
